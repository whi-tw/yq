...
func getParams(d *dataTreeNavigator, context Context, expressionNode *ExpressionNode) (string, string, error) {
	...
	if expressionNode.RHS.Operation.OperationType == blockOpType {
		...
		// we must have been given a layout and format.
		if formatString, err = getStringParamter("format", d, context.ReadOnlyClone(), expressionNode.RHS.RHS); err != nil {
			return "", "", err
		}

	} else {
		if formatString, err = getStringParamter("format", d, context.ReadOnlyClone(), expressionNode.RHS); err != nil {
			return "", "", err
		}
